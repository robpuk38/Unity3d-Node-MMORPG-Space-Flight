...
public class NetworkManager : MonoBehaviour {
	...
	{
		print ("ClientDissconnect");
		socket.Emit ("OnPlayerDisconnect");
	...
	void _OnPlayerConnected(SocketIOEvent socketIOEvent)
	{
		...
		GameObject o = GameObject.Find ("User_Row_Id_"+userJSON.Id+"_UserId_"+userJSON.UserId) as GameObject;
			if(o != null)
			{
			Debug.Log ("Debug: _OnPlayerConnected Player Is Already Online:"+ o.name);
				return;
			}
		GameObject p = Instantiate (player, Vector3.zero,Quaternion.Euler(Vector3.zero)) as GameObject;
		p.name = "User_Row_Id_"+userJSON.Id+"_UserId_"+userJSON.UserId;
	...
	void _OnPlayerMove(SocketIOEvent socketIOEvent)
	{
		...
		//	return;
		//}
		GameObject p = GameObject.Find ("User_Row_Id_"+userJSON.Id+"_UserId_"+userJSON.UserId) as GameObject;
		if(p != null)
		{
	...
	void _OnPlayerShoot(SocketIOEvent socketIOEvent)
	{
		...
		GameObject p = GameObject.Find ("User_Row_Id_"+userJSON.Id+"_UserId_"+userJSON.UserId);
		PlayerController pc = p.GetComponent<PlayerController>();
		pc.CmdFire ();*/
	}
	...
	{
		Transform _Data_Container_Canvas = p.transform.Find("Data_Container_Canvas");
